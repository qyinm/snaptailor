...
<Steps>
  <Step title="Initialize or export the manifest">
    Create a starter manifest, or export one from your existing local agent setup. `gandalf import` is a backward-compatible alias for `gandalf export`.
    ...
    `gandalf export` writes `gandalf.toml` and mirrors team skills into `.gandalf/skills/`. Use `-f` to overwrite an existing manifest. `gandalf init --from-current` remains a lighter scaffold from a local scan.
  </Step>
...
- [`gandalf export` reference](/cli/export)
